...
3. iPhone users are guided through Safari Add to Home Screen.
4. Android users get the browser install prompt when the PWA is eligible.
5. Desktop users can install from the browser address bar.
6. Advanced Android APK distribution stays separate and disabled until a signed APK is uploaded.
...
Keep `manifest.webmanifest`, `service-worker.js`, and the app start URL on the same origin. Browser install prompts usually require HTTPS, a manifest, a service worker, icons, and a user gesture.
